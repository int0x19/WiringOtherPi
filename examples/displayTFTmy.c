...
send_data_byte(0x02);// Disabled by default.
send_data_byte(0x55); //0xFF = Max brightness
send_data_byte(0x55);
send_data_byte(0x18);
...
float get_cpu_freq()
{
 ...
 char bufT[256];     // Data from device
 char tmpDataT[7];   // Temp C * 1000 reported by device
 ssize_t numRead;
 float cpufreqM = 0;
 fd = open("/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq", O_RDONLY);
  ...
  {    
     while((numRead = read(fd, bufT, 256)) > 0)
      {
       tmpDataT[7]=0;    
       strncpy(tmpDataT, &bufT[0], 7);
       cpufreqM = strtof(tmpDataT, NULL);
      }
  }
